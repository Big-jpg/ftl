...
const TIERS_WITH_FILAMENTS = TIERS.map(({ tier, label, color }) => ({
    tier,
    label,
    color,
    filaments: FILAMENTS.filter((filament) => filament.tier === tier),
}));
...
export default function HomePageContent() {
    ...
    return (
        <main
            className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-slate-100 p-6 md:p-10 flex flex-col"
        >
            <div className="max-w-6xl mx-auto w-full space-y-10 flex-1">
                ...
                <div className="space-y-12">
                    {TIERS_WITH_FILAMENTS.map(({ tier, label, color, filaments }) => (
                        <TierSection
                            key={tier}
                            tier={tier}
                            label={label}
                            color={color}
                            filaments={filaments}
                            onSelect={(filament) => setSelected(filament)}
                        />
                    ))}
                </div>
            </div>
            ...
            <FilamentModal filament={selected} onClose={() => setSelected(null)} />
        </main>
    );
}
